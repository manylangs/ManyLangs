**Conversation 재검수 프롬프트 (KR-target판, v1.2 — 채점 결과 기반 교정 생성용)**
...
이 교재는 target=한국어이며, kr은 target의 완전한 미러다. en/es/fr/pt/jp/zh/ru
7개 언어가 교정 가능 대상이고, kr을 고친다는 것은 target과의 미러 불일치를
...
재검수자가 번역 대상 7개 언어(en/es/fr/pt/jp/zh/ru) 전체를 훑으며 매번 직접
확인한다. 각 항목은 대응하는 번역 매뉴얼에 이미 있는 규칙이며, 이 절은 그
규칙이 실제로 지켜졌는지 재확인하는 체크리스트다.

* **pt — tu/você 격식 일치** (TRANSLATOR_PT.md 5장 "자연스러움 원칙"):
...
* **ru — ты/Вы 격식 일치** (TRANSLATOR_RU.md 7장): target이 캐주얼한
  ...
  안에서 ты와 Вы가 섞여 있는 것도 이 항목에서 함께 고친다.
* **제3자 성별·직업명 일치** (es/fr/pt/ru — TRANSLATOR_ES.md, TRANSLATOR_FR.md, TRANSLATOR_PT.md, TRANSLATOR_RU.md
  ...
  이 4개 언어는 그 인물 본인의 성별로만 판단한다 (화자 기준 호칭 체계를
  그대로 옮기지 않는다).
* **번역 7개 언어 공통 — 원어민 자연화 규칙 재확인**: 각 언어 매뉴얼(TRANSLATOR_EN.md, TRANSLATOR_ES.md, TRANSLATOR_FR.md, TRANSLATOR_PT.md)의 "원어민 자연화 규칙"에는 불필요한 대명사·과도한
  ...
  생성 단계(GENERATOR_KR.md)의 책임이며 이 재검수 프롬프트가
...
* 언어 키는 en/es/fr/pt/kr/jp/zh/ru 중에서만 쓴다.
...
지점을 빠짐없이 다뤘는가 / 1-1장의 상시 점검 항목(pt tu/você, ru ты/Вы,
es/fr/pt/ru 제3자 성별, 번역 7개 언어의 원어민 자연화 규칙)을 실제로
훑었는가 / kr을 미러 불일치 수정 외의
...
- 1-1장 "상시 점검 항목" 신설: pt tu/você 격식 불일치가 채점 프롬프트에서
  ...
  확인하도록 명시. 기존 TRANSLATOR_PT.md에 이미 있던 규칙이며, 새 규칙을
