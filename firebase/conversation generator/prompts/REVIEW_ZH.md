**Conversation 재검수 프롬프트 (ZH-target판, v1.1 — 채점 결과 기반 교정 생성용)**
...
`target=zh batch_id=...`가 있다. 이 batch_id를 그대로 출력에 쓴다.

이 교재는 target=중국어 간체(만다린, zh-CN)(GENERATOR_ZH.md로 생성)이며,
zh은 target의 완전한 미러다. en/es/fr/pt/kr/jp/ru 7개 언어가 교정 가능 대상이고,
zh을 고친다는 것은 target과의 미러 불일치를 바로잡는 것이지 target 자체를
...
* **점수·사유에 없어도**, 화자 일관성, 구어체 자연스러움, 존대법 적합성, target-zh
...
재검수자가 번역 대상 7개 언어(en/es/fr/pt/kr/jp/ru) 전체를 훑으며 매번 직접
...
* **제3자 성별·직업명 일치** (es/fr/pt/ru — TRANSLATOR_ES.md, TRANSLATOR_FR.md, TRANSLATOR_PT.md, TRANSLATOR_RU.md
  ...
  (참고: zh은 이 파이프라인에서 target 자신의 미러이므로 — 즉 번역이 아니라
  target과 동일한 문자열이어야 하므로 — zh 자체의 자연스러움 문제는 target
  생성 단계(GENERATOR_ZH.md)의 책임이며 이 재검수 프롬프트가
  target을 고칠 수 없는 것과 마찬가지로 zh의 표현 자체도 고치지 않는다.
  zh에 대해 이 재검수 프롬프트가 다루는 것은 오직 target과의 미러 불일치
  여부뿐이다.)
...
* 언어 키는 en/es/fr/pt/kr/jp/ru 중에서만 쓴다.
...
zh을 미러 불일치 수정 외의 이유로 건드리지 않았는가 / JSON이 아니라
...
[v1.0]
- REVIEW_PT.md의 구조를 그대로 계승 (target=zh로 교체,
  교정 가능 대상을 en/es/fr/pt/kr/jp로 조정).
- 1-1장 "상시 점검 항목" 신설: 번역 6개 언어 공통 원어민 자연화 규칙 재확인 +
  pt tu/você + fr tu/vous (기존에 확인된 격식 이원화 언어들의 재확인 항목을 사전 반영).
  zh은 이 파이프라인에서 target의 미러이므로 상시 점검 대상에서 제외.
