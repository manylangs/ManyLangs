...
function generateRuntimeJSON(chapterNum) {
  ...
  // 파일 경로
  const targetFile = path.join(C1_DIR, `conversation_${chapterId}.json`);
  const enFile = path.join(C1_DIR, `kr_en_${chapterId}.json`);
  const esFile = path.join(C1_DIR, `kr_es_${chapterId}.json`);
  const frFile = path.join(C1_DIR, `kr_fr_${chapterId}.json`);
  const ptFile = path.join(C1_DIR, `kr_pt_${chapterId}.json`);
  ...
  // Runtime JSON 파일 저장
  const outputFile = path.join(C1_DIR, `conversation_${chapterId}.runtime.json`);
...
// 메인 실행
function main() {
  // c1 폴더의 모든 conversation_*.json 파일 찾기
  const files = fs.readdirSync(C1_DIR);
