...
def synthesize(client, text: str, lang: str, speaker: str) -> AudioSegment:
...
def load_sets(json_path: Path, lang: str):
    ...
    for block in data.get("blocks", []):
        ...
        if lines:
            sets.append({"set_id": block.get("set_id", ""), "lines": lines})
...
def build_one(client, json_path: Path, wav_path: Path, cues_path: Path, lang: str):
    ...
    for set_index, block in enumerate(sets):
        ...
        for line_index, line in enumerate(block["lines"]):
            seg = synthesize(client, line["text"], lang, line["speaker"])
            final_audio += seg

            is_last_line = line_index == len(block["lines"]) - 1
...
def iter_targets(content_root: Path, lang: str, level: str, chapter: str):
    ...
    for lv in levels:
        level_dir = content_root / "conversation" / lang / lv
        ...
        for ch in chapters:
            base = level_dir / ch
            json_path = base / "data" / f"conversation_{ch}.runtime.json"
            wav_path = base / "audio" / f"conversation_{lv}_{ch}.wav"
            cues_path = base / "audio" / f"conversation_{lv}_{ch}.cues.json"
            yield lv, ch, json_path, wav_path, cues_path


def main():
    parser = argparse.ArgumentParser(description="ManyLangs Conversation TTS Builder")
    ...
    print("=" * 70)
    print("ManyLangs Conversation TTS Builder")
    print("=" * 70)
    ...
    for lv, ch, json_path, wav_path, cues_path in iter_targets(
        ...
        try:
            ...
            print(f"[OK] {lv}/{ch} sets={set_count} duration_ms={duration_ms}")
            created += 1
