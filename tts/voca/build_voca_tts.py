...
def load_sets(json_path: Path, lang: str):
    ...
    for block in data.get("blocks", []):
        ...
        word = block.get("word", {}).get(field, "").strip()
        if word:
            texts.append(word)
...
def build_one(client, json_path: Path, wav_path: Path, cues_path: Path, lang: str):
    ...
    for set_index, texts in enumerate(sets):
        ...
        for text_index, text in enumerate(texts):
            ...
            if not is_last_text:
                if text_index == 0:
                    final_audio += silence(GAP_AFTER_WORD_MS)
                else:
...
def iter_targets(content_root: Path, lang: str, level: str, chapter: str):
    ...
    for lv in levels:
        level_dir = content_root / "voca" / lang / lv
        ...
        for ch in chapters:
            ...
            wav_path = base / "audio" / f"voca_{lv}_{ch}.wav"
            cues_path = base / "audio" / f"voca_{lv}_{ch}.cues.json"
            yield lv, ch, json_path, wav_path, cues_path


def main():
    parser = argparse.ArgumentParser(description="ManyLangs Vocabulary TTS Builder")
    ...
    print("=" * 70)
    print("ManyLangs Vocabulary TTS Builder")
    print("=" * 70)
