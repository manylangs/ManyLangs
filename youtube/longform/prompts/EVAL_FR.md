**Conversation 평가(채점) 프롬프트 (FR-target판) v1.1**
...
이 교재는 GENERATOR_FR.md로 생성된 프랑스어(fr-FR, 프랑스 본토 표준어) 원문(target)을 기반으로 한다. target 언어는 fr이며, fr은 target의 완전한 미러다. 나머지 7개 언어(en/es/pt/kr/jp/zh/ru)는 target에서 직접 번역된 실제 번역이다.
...
표준 구조: 세트 열 개, 세트당 여섯 줄, 총 예순 줄, 각 줄에 target(=fr)과 여덟 개 언어(en/es/pt/kr/jp/zh/fr/ru, fr=target 미러) 포함.
