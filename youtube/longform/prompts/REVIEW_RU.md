**Conversation 재검수 프롬프트 (RU-target판, v1.0 — 채점 결과 기반 교정 생성용)**
...
`target=ru batch_id=...`가 있다. 이 batch_id를 그대로 출력에 쓴다.

이 교재는 target=러시아어(ru-RU, GENERATOR_RU.md로 생성)이며, ru는
target의 완전한 미러다. en/es/fr/pt/kr/jp/zh 7개 언어가 교정 가능 대상이고,
ru를 고친다는 것은 target과의 미러 불일치를 바로잡는 것이지 target 자체를
...
* **점수·사유에 없어도**, 화자 일관성, 구어체 자연스러움, 존대법 적합성, target-ru
...
재검수자가 번역 대상 7개 언어(en/es/fr/pt/kr/jp/zh) 전체를 훑으며 매번 직접
확인한다. 각 항목은 대응하는 번역 매뉴얼에 이미 있는 규칙이며, 이 절은 그
규칙이 실제로 지켜졌는지 재확인하는 체크리스트다.

* **번역 7개 언어 공통 — 원어민 자연화 규칙 재확인**: 각 언어 매뉴얼(대상 언어를 제외한
  TRANSLATOR_EN.md, TRANSLATOR_ES.md, TRANSLATOR_FR.md, TRANSLATOR_PT.md, TRANSLATOR_KR.md, TRANSLATOR_ZH.md, TRANSLATOR_JP.md)의 "원어민 자연화 규칙"에는 불필요한 대명사·과도한 격식·직역투를
...
* **제3자 성별·직업명 일치** (es/fr/pt — TRANSLATOR_ES.md, TRANSLATOR_FR.md, TRANSLATOR_PT.md 6-1장): 대화에
  등장하는 제3자(가족, 동료, 상사 등)를 가리키는 형용사·과거분사·직업명이 그 인물의
  실제 성별과 일치하고, 세트 안에서 중간에 바뀌지 않았는지 es/fr/pt 3개 언어에서
  확인한다. target이 러시아어의 брат/сестра처럼 인물 본인 성별로만 정하는 체계이므로,
  이 정보가 es/fr/pt로 옮겨질 때도 화자(A/B) 기준이 아니라 그 인물 본인의 성별을
  그대로 따라야 한다. (ru 자신의 성별·격식 일치는 GENERATOR_RU.md 10-1장에서
  이미 자체 확인되므로 이 재검수 프롬프트의 대상이 아니다.)

  (참고: ru는 이 파이프라인에서 target 자신의 미러이므로 — 즉 번역이 아니라
  target과 동일한 문자열이어야 하므로 — ru 자체의 자연스러움 문제는 target
  생성 단계(GENERATOR_RU.md)의 책임이며 이 재검수 프롬프트가
  target을 고칠 수 없는 것과 마찬가지로 ru의 표현 자체도 고치지 않는다.
  ru에 대해 이 재검수 프롬프트가 다루는 것은 오직 target과의 미러 불일치
  여부뿐이다.)
...
자연화 규칙, pt tu/você, fr tu/vous, es/fr/pt 제3자 성별)을 실제로 훑었는가 /
ru를 미러 불일치 수정 외의 이유로 건드리지 않았는가 / JSON이 아니라
...
[v1.0]
- REVIEW_KR.md의 구조를 그대로 계승 (target=ru로 교체,
  교정 가능 대상을 en/es/fr/pt/kr/jp/zh로 조정). GENERATOR_RU.md가
  새로 생기면서 ru가 target 언어로도 쓰일 수 있게 됨에 따라, ru-target 교재의
  재검수를 위해 신규 작성.
- 1-1장 "상시 점검 항목" 신설: 번역 7개 언어 공통 원어민 자연화 규칙 재확인 +
  pt tu/você + fr tu/vous + es/fr/pt 제3자 성별·직업명 일치(각 TRANSLATOR.md
  6-1장) — 기존 6개 target판에서 이미 검증된 상시 점검 항목을 동일하게 반영.
  ru은 이 파이프라인에서 target의 미러이므로 상시 점검 대상에서 제외하고, ru
  자신의 성별·ты/Вы 일치는 GENERATOR_RU.md 10-1장·7장에서 이미
  자체 확인됨을 명시.
