...
흐름:
  ...
  7) 나머지 언어들(target·kr 제외, ru 포함 최대 6개) 번역 + 평가/재검수 루프 (씬 단위, shorts_pipeline과 동일)
...
from voice_pool_b import VOICE_POOL_B, EXTRA_VOICE_AB, EXTRA_LANGUAGE_CODE
...
def load_eval_prompt_text(prompts_dir, target_lang):
    return find_prompt_file(prompts_dir, f"EVAL_{target_lang.upper()}.md")


def load_reviewer_prompt_text(prompts_dir, target_lang):
    return find_prompt_file(prompts_dir, f"REVIEW_{target_lang.upper()}.md")
...
def _pick_b_voice(target_lang, b_gender, scene_id):
    """A는 tts/common/config.py의 VOICE_AB[target_lang]['A'] 고정
    (없으면 voice_pool_b.py의 EXTRA_VOICE_AB로 보강, _get_a_voice 참고).
...
def _get_a_voice(tts_config, target_lang):
    """A 화자 보이스: tts/common/config.py의 VOICE_AB를 우선 조회하고,
    거기 없는 언어(예: ru -- 2026-08 기준 config.py에 아직 미등록)는
    voice_pool_b.py의 EXTRA_VOICE_AB로 보강한다. config.py에 나중에
    정식으로 추가되면 그쪽이 자동으로 우선된다."""
    if target_lang in tts_config.VOICE_AB:
        return tts_config.VOICE_AB[target_lang]["A"]
    if target_lang in EXTRA_VOICE_AB:
        return EXTRA_VOICE_AB[target_lang]["A"]
    raise ValueError(
        f"target_lang={target_lang!r}이 VOICE_AB(config.py)와 "
        f"EXTRA_VOICE_AB(voice_pool_b.py) 어디에도 없음."
    )


def _get_language_code(tts_config, target_lang):
    if target_lang in tts_config.LANGUAGE_CODE:
        return tts_config.LANGUAGE_CODE[target_lang]
    if target_lang in EXTRA_LANGUAGE_CODE:
        return EXTRA_LANGUAGE_CODE[target_lang]
    raise ValueError(
        f"target_lang={target_lang!r}이 LANGUAGE_CODE(config.py)와 "
        f"EXTRA_LANGUAGE_CODE(voice_pool_b.py) 어디에도 없음."
    )
...
def tts_generate_scene_audio(lines, target_lang, b_gender, scene_id, monologue, out_audio_path,
    ...
    a_voice = _get_a_voice(tts_config, target_lang)
    b_voice = None if monologue else _pick_b_voice(target_lang, b_gender, scene_id)
    language_code = _get_language_code(tts_config, target_lang)
...
# ---------------------------------------------------------------------------
# 6) 나머지 언어들(target·kr 제외, ru 포함 최대 6개) 번역 + 평가/재검수 루프 (씬 단위, shorts_pipeline 로직 재사용)
# ---------------------------------------------------------------------------
