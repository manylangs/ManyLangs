**Idiom 재검수 프롬프트 (EN-target판 — 채점 결과 기반 교정 생성용)**
...
`target=en batch_id=...`가 있다. 이 batch_id를 그대로 출력에 쓴다.

이 교재는 target=영어이며, en은(는) target의 완전한 미러다. kr/es/fr/pt/zh/jp/ru
7개 언어가 교정 가능 대상이고, en을(를) 고친다는 것은 target과의 미러 불일치를
...
**탈이디엄화 규칙**: kr/es/fr/pt/zh/jp/ru 7개 언어는 explanation과 example
...
* **점수·사유에 없어도**, explanation 정확성, function 태그별 레지스터
  일관성(natural_spoken_example만 캐주얼), target-en 미러 일치 등 명백히
...
* 언어 키는 kr/es/fr/pt/zh/jp/ru 중에서만 쓴다 (target도 이론상 수정
