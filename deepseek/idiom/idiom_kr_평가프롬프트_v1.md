**Idiom 최종 채점 프롬프트 (KR-target판, v1.0 — DeepSeek 교차검증 3단계용)**
...
이 교재는 target=Korean 파이프라인이다. `10_KR_IDIOM_TARGET_GENERATOR.md`가 만든 한국어 원문(target, expression 포함)을 `11_EN_IDIOM_TRANSLATOR.md`가 영어(en)로 번역하고, `12~16_*_IDIOM_TRANSLATOR.md`가 그 en에서 es/fr/pt/jp/zh로 각각 번역하며, `17_KR_IDIOM_TRANSLATOR.md` [A] 분기에 따라 kr은 target의 완전한 미러다.

**이디엄 파이프라인의 구조적 핵심**: `expression` 필드는 target 하나만 존재한다. 언어 간 "이디엄 대 이디엄" 매칭은 이 설계에서 애초에 시도하지 않는다. explanation과 examples는 모든 언어에 존재하지만, **실제 관용구·속담·사자성어 형태를 진짜로 담고 있어도 되는 언어는 target(=kr) 하나뿐**이다. en/es/fr/pt/jp/zh 6개 언어는 target이 전달하는 의미를 그 언어의 대응 관용구로 치환하지 않고 반드시 자연스러운 일반 서술로 풀어야 한다(`11_EN_IDIOM_TRANSLATOR.md` 이하 전 TRANSLATOR류의 [이디엄 처리 원칙], v3.0부터 절대 규칙). 이 채점 프롬프트의 가장 중요한 임무 중 하나는 바로 이 규칙이 실제로 지켜졌는지 확인하는 것이다(Section 2의 영역 ⑥).
...
* 실제 폴더 경로 (예: `content/idiom/kr/a1/001/data/data.json`)
...
* 채점은 "완벽한가"가 아니라 "시중에서 실제로 판매·서비스되는 최상위 한국어 관용표현/이디엄 학습 콘텐츠와 비교했을 때 몇 점인가"를 기준으로 한다.
* 비교 대상(참고용 벤치마크): TOPIK 관용어·속담 관련 공식/준공식 학습 자료, 세종한국어 관용표현 단원, TTMIK의 이디엄·슬랭 콘텐츠, Duolingo Korean, 네이버 사전의 관용구 학습 콘텐츠. 실시간 정보가 없다면 일반 지식 기반 추정임을 `benchmark.note`에 명시한다.
...
5개 function(basic_meaning/situational_application/extended_meaning/natural_spoken_example/learner_friendly_simple) 각각이 실제 한국어 원어민이 쓸 법한 문장인가. 각 예문이 실제로 target 이디엄 표현을 포함하는가. 각 예문이 explanation과 같은 의미를 가리키는가.

**④ 문법·표기 정확성 (가중치 8)**
표준어(서울·경기) 준수, 사투리 없음, 맞춤법·띄어쓰기 오류 없음, 5개 예문이 서로 다른 문장 구조를 사용하는가(단순 반복 방지).

**⑤ Function 태그별 레지스터 일관성 (가중치 8)**
natural_spoken_example만 캐주얼한 반말/구어체이고, 나머지 4개(basic_meaning/situational_application/extended_meaning/learner_friendly_simple)는 해요체 등 중립적 회화체를 유지하는가. 한 예문 안에서 반말/존댓말이 섞이지 않는가.

**⑥ 탈이디엄화(De-idiomatization) 검증 — en/es/fr/pt/jp/zh 6개 언어 전용 (가중치 20)**
이 영역은 이 채점 프롬프트에서 가장 중요한 검증이며, 반드시 이디엄별·언어별로 개별 확인한다.

절차:
1) `expression.target`이 실제 한국어 관용구·속담·사자성어인지 확인한다 (거의 항상 그렇다 — 이는 정상이며 target 자체는 수정 대상이 아니다).
2) en/es/fr/pt/jp/zh 6개 언어 각각의 explanation과 5개 examples 전체(이디엄당 6개 언어 × 6개 필드 = 36개 지점, 배치 전체로는 5개 이디엄 × 36 = 180개 지점)를 확인한다.
3) 각 지점에서 그 언어의 **완성된 고정 관용구·속담·성어·諺(예: 영어 "a piece of cake", 중국어 성어, 일본어 사자성어·속담, 스페인어/프랑스어/포르투갈어의 관용적 숙어 전체)를 통째로 사용했는지** 확인한다.
4) 사용했다면 위반(violation)으로 기록한다. target의 의미를 해당 언어의 자연스러운 **일반 서술**로 풀어썼다면 정상이다.
5) 부분적 은유(예: 한 단어짜리 흔한 비유 표현)와 "완성된 고정 관용구 전체"를 구분한다 — 언어 자체의 일반적인 비유적 어휘 선택까지 과도하게 위반으로 잡지 않는다. 판단 기준은 "그 표현이 사전에 관용구/속담 항목으로 통째로 등재되어 있는가"이다.

이 영역의 점수는 다음 식으로 계산한다: `score = 10 - (violation_count * 10 / total_checked_points)`를 참고 기준으로 삼되, 최소 1건이라도 위반이 있으면 8점을 넘지 않는다. 위반이 3건 이상이면 즉시 blocking_issue로도 등록한다(아래 Section 4).
...
**⑨ 미러 일치도 (가중치 6)**
`explanation.kr`과 `examples[].kr`이 `explanation.target`/`examples[].target`과 완전히 동일한가(target=한국어이므로 kr은 target의 완전한 미러여야 한다).
...
* 영역 ⑥(탈이디엄화)에서 3건 이상의 위반 — 즉 en/es/fr/pt/jp/zh 중 어디서든 해당 언어의 완성된 관용구·속담·성어로 치환한 사례가 배치 전체에서 3건 이상
* kr 미러가 target과 불일치
...
```json
{
  ...
  "benchmark": {
    "references": ["TOPIK 관용어 학습자료", "TTMIK 이디엄 콘텐츠", "Duolingo Korean"],
