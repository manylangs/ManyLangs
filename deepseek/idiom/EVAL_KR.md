**Idiom 최종 채점 프롬프트 (KR-target판 — DeepSeek 교차검증 3단계용)**
...
이 교재는 target=Korean 파이프라인이다. `10_KR_IDIOM_TARGET_GENERATOR.md`가 만든 한국어 원문(target, expression 포함)을 `11_EN_IDIOM_TRANSLATOR.md`가 영어(en)로 번역하고, `12~16_*_IDIOM_TRANSLATOR.md`가 그 en에서 es/fr/pt/jp/zh로 각각 번역하며, ru는 target에서 직접 번역된다. kr은 target의 완전한 미러다.

**이디엄 파이프라인의 구조적 핵심**: `expression` 필드는 target 하나만 존재한다. 언어 간 "이디엄 대 이디엄" 매칭은 이 설계에서 애초에 시도하지 않는다. explanation과 examples는 모든 언어에 존재하지만, **실제 관용구·속담·성어 형태를 진짜로 담고 있어도 되는 언어는 target(=kr) 하나뿐**이다. en/es/fr/pt/zh/jp/ru 7개 언어는 target이 전달하는 의미를 그 언어의 대응 관용구로 치환하지 않고 반드시 자연스러운 일반 서술로 풀어야 한다(TRANSLATOR류의 [이디엄 처리 원칙], 절대 규칙). **target=kr 자체가 실제 한국어 관용구를 담고 있는 것은 정상이며 의도된 설계다 — target을 위반으로 잘못 표시하지 않는다.** 이 채점 프롬프트의 가장 중요한 임무 중 하나는 나머지 7개 언어에서 이 규칙이 실제로 지켜졌는지 확인하는 것이다(Section 2의 영역 ⑥).
...
"explanation": { "target": "...", "en": "...", "es": "...", "fr": "...", "pt": "...", "zh": "...", "jp": "...", "ru": "..." },
"examples": [
{ "function": "basic_meaning", "target": "...", "en": "...", "es": "...", "fr": "...", "pt": "...", "zh": "...", "jp": "...", "ru": "..." },
...
* 비교 대상(참고용 벤치마크): TOPIK 관용어 학습자료, TTMIK 이디엄 콘텐츠, Duolingo Korean, 네이버 사전 관용구 콘텐츠. 실시간 정보가 없다면 일반 지식 기반 추정임을 `benchmark.note`에 명시한다.
...
natural_spoken_example만 캐주얼한 반말/구어체이고, 나머지 4개는 해요체 등 중립적 회화체를 유지하는가. 한 예문 안에서 반말/존댓말이 섞이지 않는가.

**⑥ 탈이디엄화(De-idiomatization) 검증 — en/es/fr/pt/zh/jp/ru 7개 언어 전용 (가중치 20)**
이 영역은 이 채점 프롬프트에서 가장 중요한 검증이며, 반드시 이디엄별·언어별로 개별 확인한다.

절차:
1) `expression.target`이 실제 한국어 관용구·속담·성어인지 확인한다 (거의 항상 그렇다 — 이는 정상이며 target 자체는 수정 대상이 아니다).
2) en/es/fr/pt/zh/jp/ru 7개 언어 각각의 explanation과 5개 examples 전체(이디엄당 7개 언어 × 6개 필드 = 42개 지점, 배치 전체로는 5개 이디엄 × 42 = 210개 지점)를 확인한다.
3) 각 지점에서 그 언어의 **완성된 고정 관용구·속담·성어(사자성어·속담 (예: "누워서 떡 먹기")처럼 target에서 발견되면 안 되는 것과 동일한 종류를, 나머지 언어 각각에서)를 통째로 사용했는지** 확인한다.
...
**⑨ 미러 일치도 (가중치 6)**
`explanation.kr`과 `examples[].kr`이 `explanation.target`/`examples[].target`과 완전히 동일한가(target=한국어이므로 kr은(는) target의 완전한 미러여야 한다).
...
* 영역 ⑥(탈이디엄화)에서 3건 이상의 위반 — 즉 en/es/fr/pt/zh/jp/ru 중 어디서든 해당 언어의 완성된 관용구·속담·성어로 치환한 사례가 배치 전체에서 3건 이상
* kr 미러가 target과 불일치
...
```json
{
  ...
  "benchmark": {
    "references": ["TOPIK 관용어 학습자료", "TTMIK 이디엄 콘텐츠", "Duolingo Korean", "네이버 사전 관용구 콘텐츠"],
