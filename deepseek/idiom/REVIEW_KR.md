**Idiom 재검수 프롬프트 (KR-target판 — 채점 결과 기반 교정 생성용)**
...
이 교재는 target=한국어이며, kr은(는) target의 완전한 미러다. en/es/fr/pt/zh/jp/ru
7개 언어가 교정 가능 대상이고, kr을(를) 고친다는 것은 target과의 미러 불일치를
...
**탈이디엄화 규칙**: en/es/fr/pt/zh/jp/ru 7개 언어는 explanation과 example
...
* 언어 키는 en/es/fr/pt/zh/jp/ru 중에서만 쓴다 (target도 이론상 수정
