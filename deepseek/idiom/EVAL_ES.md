**Idiom 최종 채점 프롬프트 (ES-target판 — DeepSeek 교차검증 3단계용)**
...
이 교재는 idiom 시리즈의 ES-target 생성 파이프라인으로 생성된 스페인어(es-ES(스페인 본토 표준어)) 원문(target, expression 포함)을 기반으로 한다. es은(는) target의 완전한 미러다. 나머지 7개 언어(kr/en/fr/pt/zh/jp/ru)는 이 시리즈의 실제 생성 파이프라인 설계(직접 번역 또는 피벗 경유)에 따라 target에서 번역된 결과이며, 이 채점 프롬프트는 번역 경로와 무관하게 최종 결과물의 언어별 독립 품질만을 기준으로 채점한다.

**이디엄 파이프라인의 구조적 핵심**: `expression` 필드는 target 하나만 존재한다. 언어 간 "이디엄 대 이디엄" 매칭은 이 설계에서 애초에 시도하지 않는다. explanation과 examples는 모든 언어에 존재하지만, **실제 관용구·속담·성어 형태를 진짜로 담고 있어도 되는 언어는 target(=es) 하나뿐**이다. kr/en/fr/pt/zh/jp/ru 7개 언어는 target이 전달하는 의미를 그 언어의 대응 관용구로 치환하지 않고 반드시 자연스러운 일반 서술로 풀어야 한다(TRANSLATOR류의 [이디엄 처리 원칙], 절대 규칙). **target=es 자체가 실제 스페인어 관용구를 담고 있는 것은 정상이며 의도된 설계다 — target을 위반으로 잘못 표시하지 않는다.** 이 채점 프롬프트의 가장 중요한 임무 중 하나는 나머지 7개 언어에서 이 규칙이 실제로 지켜졌는지 확인하는 것이다(Section 2의 영역 ⑥).
...
* 실제 폴더 경로 (예: `content/idiom/es/a1/001/data/data.json`)
...
"frequency_rank": 1,
"frequency_stars": "★★★★★",
"expression": { "target": "pan comido" },
"explanation": { "target": "...", "kr": "...", "en": "...", "fr": "...", "pt": "...", "zh": "...", "jp": "...", "ru": "..." },
"examples": [
{ "function": "basic_meaning", "target": "...", "kr": "...", "en": "...", "fr": "...", "pt": "...", "zh": "...", "jp": "...", "ru": "..." },
...
* 채점은 "완벽한가"가 아니라 "시중에서 실제로 판매·서비스되는 최상위 스페인어 관용표현/이디엄 학습 콘텐츠와 비교했을 때 몇 점인가"를 기준으로 한다.
* 비교 대상(참고용 벤치마크): SpanishDict Idioms, Instituto Cervantes 관용표현 자료, Duolingo Spanish. 실시간 정보가 없다면 일반 지식 기반 추정임을 `benchmark.note`에 명시한다.
...
5개 function(basic_meaning/situational_application/extended_meaning/natural_spoken_example/learner_friendly_simple) 각각이 실제 스페인어 원어민이 쓸 법한 문장인가. 각 예문이 실제로 target 이디엄 표현을 포함하는가. 각 예문이 explanation과 같은 의미를 가리키는가.

**④ 문법·표기 정확성 (가중치 8)**
es-ES 본토 표준 철자·문법 준수, 중남미식 표현(vos 등) 없음. 5개 예문이 서로 다른 문장 구조를 사용하는가(단순 반복 방지).

**⑤ Function 태그별 레지스터 일관성 (가중치 8)**
natural_spoken_example만 캐주얼한 tú 구어체이고, 나머지 4개는 중립적 회화체(과도한 usted 격식 없이)를 유지하는가. 한 예문 안에서 격식 수준이 섞이지 않는가.

**⑥ 탈이디엄화(De-idiomatization) 검증 — kr/en/fr/pt/zh/jp/ru 7개 언어 전용 (가중치 20)**
이 영역은 이 채점 프롬프트에서 가장 중요한 검증이며, 반드시 이디엄별·언어별로 개별 확인한다.

절차:
1) `expression.target`이 실제 스페인어 관용구·속담·성어인지 확인한다 (거의 항상 그렇다 — 이는 정상이며 target 자체는 수정 대상이 아니다).
2) kr/en/fr/pt/zh/jp/ru 7개 언어 각각의 explanation과 5개 examples 전체(이디엄당 7개 언어 × 6개 필드 = 42개 지점, 배치 전체로는 5개 이디엄 × 42 = 210개 지점)를 확인한다.
3) 각 지점에서 그 언어의 **완성된 고정 관용구·속담·성어("pan comido" 같은 완성 관용구처럼 target에서 발견되면 안 되는 것과 동일한 종류를, 나머지 언어 각각에서)를 통째로 사용했는지** 확인한다.
...
**⑨ 미러 일치도 (가중치 6)**
`explanation.es`과 `examples[].es`이 `explanation.target`/`examples[].target`과 완전히 동일한가(target=스페인어이므로 es은(는) target의 완전한 미러여야 한다).
...
* 영역 ⑥(탈이디엄화)에서 3건 이상의 위반 — 즉 kr/en/fr/pt/zh/jp/ru 중 어디서든 해당 언어의 완성된 관용구·속담·성어로 치환한 사례가 배치 전체에서 3건 이상
* es 미러가 target과 불일치
...
```json
{
  ...
  "benchmark": {
    "references": ["SpanishDict Idioms", "Instituto Cervantes 관용표현 자료", "Duolingo Spanish"],
