**Idiom 최종 채점 프롬프트 (EN-target판, v1.0 — DeepSeek 교차검증 3단계용)**
...
이 교재는 target=English 파이프라인이다. `09_EN_IDIOM_TARGET_GENERATOR.md`가 만든 영어 원문(target, expression 포함, 실제 영어 관용구·속담)이 그대로 en 컬럼의 완전한 미러가 된다(이 target=영어판에서는 `11_EN_IDIOM_TRANSLATOR.md`가 호출되지 않는다 — target=한국어판에서만 쓰이는 문서다). `12~16_*_IDIOM_TRANSLATOR.md`가 en(=target)에서 es/fr/pt/jp/zh로 각각 번역하며, `17_KR_IDIOM_TRANSLATOR.md` [B] 분기에 따라 kr도 en(=target)에서 직접 번역된다.

**이디엄 파이프라인의 구조적 핵심**: `expression` 필드는 target 하나만 존재한다. 언어 간 "이디엄 대 이디엄" 매칭은 이 설계에서 애초에 시도하지 않는다. explanation과 examples는 모든 언어에 존재하지만, **실제 관용구·속담 형태를 진짜로 담고 있어도 되는 언어는 target(=en) 하나뿐**이다. es/fr/pt/jp/zh/kr 6개 언어는 en이 전달하는 의미를 그 언어의 대응 관용구로 치환하지 않고 반드시 자연스러운 일반 서술로 풀어야 한다(`12~17번 TRANSLATOR류`의 [이디엄 처리 원칙], 절대 규칙). **이 target=영어판에서는 en 자체가 실제 영어 관용구를 담고 있는 것이 정상이며 의도된 설계다 — en을 위반으로 잘못 표시하지 않는다.** 이 채점 프롬프트의 가장 중요한 임무 중 하나는 나머지 6개 언어에서 이 규칙이 실제로 지켜졌는지 확인하는 것이다(Section 2의 영역 ⑥).
...
* 실제 폴더 경로 (예: `content/idiom/en/a1/001/data/data.json`)
...
"frequency_rank": 1,
"frequency_stars": "★★★★★",
"expression": { "target": "a piece of cake" },
"explanation": { "target": "...", "en": "...", "es": "...", "fr": "...", "pt": "...", "kr": "...", "zh": "...", "jp": "..." },
"examples": [
{ "function": "basic_meaning", "target": "...", "en": "...", "es": "...", "fr": "...", "pt": "...", "kr": "...", "zh": "...", "jp": "..." },
...
* 채점은 "완벽한가"가 아니라 "시중에서 실제로 판매·서비스되는 최상위 영어 관용표현/이디엄 학습 콘텐츠와 비교했을 때 몇 점인가"를 기준으로 한다.
* 비교 대상(참고용 벤치마크): Cambridge Dictionary의 idiom 콘텐츠, Oxford Idioms Dictionary 계열 학습서, EBS/YBM 영어 관용표현 교재, Duolingo English, 스픽(Speak)류 AI 스피킹 앱의 이디엄 콘텐츠. 실시간 정보가 없다면 일반 지식 기반 추정임을 `benchmark.note`에 명시한다.
...
`explanation.target`이 4장 확정 목록과 정확히 일치하는가(변형·의역 없음). explanation이 이디엄의 비유적 의미를 학습자가 이해할 수 있는 사전 뜻풀이 문체로 명확히 전달하는가.

**③ 예문 자연스러움·원어민성 (가중치 12)**
5개 function(basic_meaning/situational_application/extended_meaning/natural_spoken_example/learner_friendly_simple) 각각이 실제 미국 영어 원어민이 쓸 법한 문장인가. 각 예문이 실제로 target 이디엄 표현을 포함하는가. 각 예문이 explanation과 같은 의미를 가리키는가.

**④ 문법·표기 정확성 (가중치 8)**
철자·구두점 오류가 없는가. 5개 예문이 서로 다른 문장 구조를 사용하는가(단순 반복 방지). 주어 누락 등 English 문법 오류가 없는가.

**⑤ Function 태그별 레지스터 일관성 (가중치 8)**
natural_spoken_example만 캐주얼한 구어체이고, 나머지 4개(basic_meaning/situational_application/extended_meaning/learner_friendly_simple)는 중립적 회화체를 유지하는가.

**⑥ 탈이디엄화(De-idiomatization) 검증 — es/fr/pt/jp/zh/kr 6개 언어 전용 (가중치 20)**
이 영역은 이 채점 프롬프트에서 가장 중요한 검증이며, 반드시 이디엄별·언어별로 개별 확인한다.

절차:
1) `expression.target`(=en)이 실제 영어 관용구·속담인지 확인한다 (거의 항상 그렇다 — 이는 이 target=영어판에서 정상이며, target/en 자체는 수정 대상이 아니다).
2) es/fr/pt/jp/zh/kr 6개 언어 각각의 explanation과 5개 examples 전체(이디엄당 6개 언어 × 6개 필드 = 36개 지점, 배치 전체로는 5개 이디엄 × 36 = 180개 지점)를 확인한다.
3) 각 지점에서 그 언어의 **완성된 고정 관용구·속담·성어(예: 스페인어 "pan comido", 중국어 "小菜一碟" 같은 성어, 일본어 사자성어·속담, 프랑스어/포르투갈어의 관용적 숙어, 한국어 사자성어·속담 전체)를 통째로 사용했는지** 확인한다.
4) 사용했다면 위반(violation)으로 기록한다. en(=target)의 의미를 해당 언어의 자연스러운 **일반 서술**로 풀어썼다면 정상이다 (예: "pan comido" 대신 "muy fácil de hacer").
...
**⑨ 미러 일치도 (가중치 6)**
`explanation.en`과 `examples[].en`이 `explanation.target`/`examples[].target`과 완전히 동일한가(target=영어이므로 en은 target의 완전한 미러여야 한다).
...
* 영역 ⑥(탈이디엄화)에서 3건 이상의 위반 — 즉 es/fr/pt/jp/zh/kr 중 어디서든 해당 언어의 완성된 관용구·속담·성어로 치환한 사례가 배치 전체에서 3건 이상
* en 미러가 target과 불일치
* `expression.target`이 210개 확정 목록과 다른 표현으로 바뀌어 있음
...
```json
{
  ...
  "domains": [
    ...
    {"key": "de_idiomatization", "name": "탈이디엄화 검증 (6개 학습언어)", "weight": 20, "score": 0,
      "violations": [
  ...
  "benchmark": {
    "references": ["Cambridge Dictionary idioms", "Oxford Idioms Dictionary", "Duolingo English"],
...
출력 전에 확인한다: batch_id를 임의로 지어내지 않았는가 / 10개 영역 모두 채점했는가 / 가중치 합이 100인가 / final_score 계산식이 맞는가 / 영역 ⑥을 위해 5개 이디엄 × 6개 언어 × (explanation 1 + examples 5) = 180개 지점을 실제로 하나하나 확인했는가, 아니면 대충 훑고 점수만 매기지 않았는가 / 위반을 찾았다면 found_expression에 실제 발견된 표현을 구체적으로 적었는가 / target=en 자체(실제 영어 관용구)를 위반으로 잘못 표시하지 않았는가 / 3건 이상 위반인데 PASS로 표시하지 않았는가 / decision이 Section 4 규칙과 일치하는가 / JSON 외의 텍스트를 출력하지 않았는가 / 벤치마크 점수가 사실인 것처럼 과도하게 단정되지 않았는가.
