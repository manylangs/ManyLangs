#!/usr/bin/env python3
"""
conversation_eval_pipeline.py

conversation generator/data (review_conversation.py와 동일한 원본 경로)를
순회하며 DeepSeek로 채점하고, 결과를 파일로 저장하지 않고 터미널에
batch_id 순서대로 출력한다 (voca/real용 eval_pipeline과 동일한 구조).

conversation 시리즈는 voca처럼 target/미러 개념이 있다 (kr판/en판,
--target-lang 필수). domain_scores는 real처럼 dict 형태, weight 합계는
100. review_conversation.py의 교체 키는
  REPLACEMENTS: (set_id, line_number, lang) -> "문장"
  TITLE_REPLACEMENTS: {lang: "제목"}
이 둘이며, voca와 달리 word/meaning_zone 개념이 없다.

사용법
------
  export DEEPSEEK_API_KEY=sk-xxxx
  python conversation_eval_pipeline.py \
  ...
  옵션:
    --root PATH           conversation generator/data 경로
    --target-lang kr|en   이 폴더 전체의 target 언어 (필수)
    --prompt-dir PATH     평가 프롬프트(.md)가 있는 폴더
    --prompt-file NAME    평가 프롬프트 파일명 (기본: conversation_{target-lang}_평가프롬프트_v1.md)
    --batch 007,010-015   특정 batch_id만 (기본: 전체, 폴더 존재하는 것만)
...
DEFAULT_ROOT = Path(
    "/Users/junghasuk/Desktop/ManyLangs/web/firebase/conversation generator/data"
)
...
# ---------------------------------------------------------------------------
# 1. 배치 스캔 — review_conversation.py의 resolve_json_file()과 동일한 경로
# ---------------------------------------------------------------------------
...
def scan_batches(root: Path, batch_filter):
    ...
    for batch_dir in sorted(root.iterdir()):
        ...
        if batch_filter and batch_id not in batch_filter:
            continue
        fp = batch_dir / f"conversation_{batch_id}.runtime.json"
        if fp.is_file():
...
# ---------------------------------------------------------------------------
# 3. 검증 및 게이트 (domain_scores dict, blocking_issues 객체: set_id/line_number/lang)
# ---------------------------------------------------------------------------
...
def build_review_block_text(target_lang: str, batch_id: str, batch_json: dict, result: dict) -> str:
    lines = []
    lines.append(f"# 재검수 요청 — series=conversation target={target_lang} batch_id={batch_id}")
    lines.append("")
    ...
    if blocking:
        ...
        for b in blocking:
            lines.append(
                f"- [{b.get('domain')}] set_id={b.get('set_id')} "
                f"line={b.get('line_number')} lang={b.get('lang')}: {b.get('issue')}"
            )
...
def run(root: Path, prompt_dir: Path, prompt_file: str, target_lang: str, batch_filter,
    ...
    print(f"[스캔 결과] {len(entries)}개 배치 발견 (series=conversation target_lang={target_lang})")
    ...
    if review_batches:
        ...
        print("conversation_kr/en_재검수프롬프트_v1.0.md 세션에 붙여넣으세요.")
        print("=" * 80)
...
def parse_args():
    p = argparse.ArgumentParser(description="conversation generator/data 원본 기준 DeepSeek 채점 파이프라인 (결과는 터미널 출력)")
    p.add_argument("--root", default=str(DEFAULT_ROOT), help="conversation generator/data 경로")
    ...
    p.add_argument("--prompt-file", default=None, help="평가 프롬프트 파일명 (기본: conversation_{target-lang}_평가프롬프트_v1.md)")
    p.add_argument("--batch", default="", help="예: 007,010-015 (기본: 전체)")
...
if __name__ == "__main__":
    args = parse_args()
    prompt_file = args.prompt_file or f"conversation_{args.target_lang}_평가프롬프트_v1.1md"
    run(
        root=Path(args.root),
