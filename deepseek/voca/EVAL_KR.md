**Voca 최종 채점 프롬프트 (KR-target판 — 버전 참조 갱신, FLAG 처리 추가, orphan flag 게이트 완화)**
...
이 교재는 `kr_voca_manual_A_target_generation_v3.md`로 생성된 한국어 원문(target)을 기반으로 하며, target=Korean, LANG_GROUP=AGGLUTINATIVE_SOV (Type A: 주어 생략 가능, SOV 어순, 교착어)다. `word.kr`과 `examples[].kr`은 target의 완전한 미러이며, en/es/fr/pt/zh/jp/ru는 각 언어의 Universal Translation Prompt(`translation_prompt_common_*_v3.md`)로 생성된 번역이다.
...
* 실제 폴더 경로 (예: `content/voca/kr/a1/003/data/data.json`)
...
data.json 구조는 `meta{series,level,id}`, `title{target, en, es, fr, pt, zh, jp, ru}`, `blocks[].id`(block_001~005), `blocks[].word`(언어별 core+meaning_zone), `blocks[].examples[]`(3개, declarative→negative→question, 언어별 문자열)로 고정되어 있다.
...
* 비교 대상(참고용 벤치마크)은 다음과 같은 실제 상위권 한국어 학습 서비스/교재를 염두에 둔다: TOPIK 공식 기출·연계 교재, 세종한국어, Talk To Me In Korean(TTMIK), Duolingo Korean. 정확한 실시간 정보가 없다면 일반적으로 알려진 이 서비스들의 강점(자연스러움, 상황 다양성, 난이도 위계, 학습 설계)을 기준으로 합리적으로 추정하고, 추정치임을 `benchmark.note`에 명시한다. 없는 사실을 정밀한 수치처럼 단정하지 않는다.
...
실제 한국어 원어민이 구어에서 쓸 법한 문장인가, 번역투가 남아있지 않은가. 어순·시제·관용적 표현이 자연스러운가.
...
**⑧ 미러 일치도 (가중치 5)**
`word.kr`과 `examples[].kr`이 `word.target`/`examples[].target`과 완전히 동일한가 (target=한국어이므로 kr은(는) target의 미러).
...
* **helper 언어(en/es/fr/pt/zh/jp/ru) 예문에서 사역·설득·허용·요청 등 다중 술어 구조의 부정·가능/불가능·양태가 target과 다른 술어에 걸려 실질적 오역이 된 경우 (negation/modality scope 오류).** 이 프롬프트는 helper 언어 번역 품질 자체를 채점 대상으로 삼지 않지만(Section 6 자체확인 참고), 채점 과정에서 이런 오류를 우연히 발견하면 그 자체로 blocking_issue로 기록한다 — target=kr 컬럼 채점과는 별개의 안전망이다.
...
```json
{
  ...
  "benchmark": {
    "references": ["TOPIK 공식 기출·연계 교재", "세종한국어", "Talk To Me In Korean(TTMIK)", "Duolingo Korean"],
