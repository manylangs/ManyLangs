**Voca 최종 채점 프롬프트 (EN-target판, v2.0 — 버전 참조 갱신, FLAG 처리 추가, orphan flag 게이트 완화)**
...
이 교재는 `en_voca_manual_A_target_generation_v3.md`로 생성된 영어 원문(target)을 기반으로 하며, target=English, LANG_GROUP=ANALYTIC_SVO (Type B: 주어 필수, SVO 어순, 분석어)다. `word.en`과 `examples[].en`은 target의 완전한 미러이며, es/fr/pt/jp/zh/kr는 각 언어의 Universal Translation Prompt(`translation_prompt_common_*_v3.md`)로 생성된 번역이다.
...
* 실제 폴다 경로 (예: `content/voca/en/a1/003/data/data.json`)
...
* 채점은 "이 교재가 절대적으로 완벽한가"가 아니라 "시중에서 실제로 판매·서비스되는 최상위 영어 학습 콘텐츠와 비교했을 때 몇 점인가"를 기준으로 한다.
* 비교 대상(참고용 벤치마크)은 다음과 같은 실제 상위권 영어 학습 서비스/교재를 염두에 둔다: Cambridge/Oxford 공인 교재(Cambridge English, Oxford Bookworms), EBS·YBM 계열 국내 영어 교재, Duolingo English, 스픽(Speak)·시원스쿨 등 국내 AI 스피킹 앱의 예문 품질. 정확한 실시간 정보가 없다면 일반적으로 알려진 이 서비스들의 강점(자연스러움, 상황 다양성, 난이도 위계, 학습 설계)을 기준으로 합리적으로 추정하고, 추정치임을 `benchmark.note`에 명시한다. 없는 사실을 정밀한 수치처럼 단정하지 않는다.
...
**① 어휘·개념 선정 적합성 (가중치 10)**
ConceptID가 실제 영어 개념을 정확히 대표하는가(영어 단어 형태가 아니라 개념 우선 설계 원칙에 부합하는가). CEFR 레벨(A1~C2)에 맞는 난이도인가. 12-domain 균형과 corpus-frequency 기반 선정 취지에 부합하는가.

**② Core & Meaning Zone 품질 (가중치 15)**
`en_voca_manual_A` Section 3.4의 10개 Meaning Zone Rule을 위반 없이 지켰는가. 다의어 처리가 정당한 의미 분기인지 확인한다(Section 3.2 Polysemy Rule). **이 영역은 meaning_zone 자체의 사전적 타당성만 본다 — 예문에서 core나 meaning_zone이 실제로 쓰였는지는 이 영역의 채점 대상이 아니다 (그건 별도 스캐너 단계 소관).**
...
* 제외 대상으로 판단한 단어/표현이 같은 block의 `examples[].en` (및 필요시 다른 언어 컬럼) 안에 그대로 등장하는가?
...
실제 미국 영어 원어민이 구어에서 쓸 법한 문장인가, 번역투가 남아있지 않은가. 어순·시제·관용적 표현이 자연스러운가.

**④ 문법·표기 정확성 (가중치 10)**
Rule EN-01(주어 필수)이 모든 예문에서 지켜졌는가. A1/A2에서 부정문이 축약형(I'm not, doesn't 등)으로 자연스럽게 쓰였는가. 철자·구두점 오류가 없는가.
...
문화 특정적 상황이 🚩 CULTURAL 플래그 없이 쓰이지 않았는가. 로맨틱 뉘앙스 등 불필요한 의미적 모호성(예: "go out with a friend")이 없는가.

**⑦ TTS·표기 규칙 적합성 (가중치 10)**
숫자·시간·단위 표기가 TTS 엔진이 정확히 읽을 수 있는 형태인가. 이모지/이모티콘이 없는가. 문장부호가 TTS 휴지(pause)에 적합한가.

**⑧ 미러 일치도 (가중치 5)**
`word.en`과 `examples[].en`이 `word.target`/`examples[].target`과 완전히 동일한가 (target=영어이므로 en은 target의 미러).

**⑨ 시장 경쟁력 (가중치 15)**
학습 설계(난이도 곡선, 반복·확장 구조), 콘텐츠 밀도(한 배치 5개 단어로 학습 효과를 낼 수 있는 예문 구성인지), 시중 최상위 영어 학습 서비스와 비교했을 때 예문·의미망 설명의 매력도와 학습 효율.
...
* 주어 누락 등 Type B 언어 필수 규칙 위반이 다수 남아있는 경우
* 이모지·비TTS친화 표기가 다수 남아있어 TTS 단계 진행이 불가능한 수준
* **helper 언어(kr/es/fr/pt/jp/zh) 예문에서 사역·설득·허용·요청 등 다중 술어 구조의 부정·가능/불가능·양태가 target과 다른 술어에 걸려 실질적 오역이 된 경우 (negation/modality scope 오류).** 이 프롬프트는 helper 언어 번역 품질 자체를 채점 대상으로 삼지 않지만(Section 6 자체확인 참고), 채점 과정에서 이런 오류를 우연히 발견하면 그 자체로 blocking_issue로 기록한다 — target=en 컬럼 채점과는 별개의 안전망이다.
...
```json
{
  ...
  "benchmark": {
    "references": ["Cambridge English", "EBS/YBM", "Duolingo English"],
  ...
  "meaning_zone_orphan_flags": [
    {
      ...
      "reason_excluded": "",
      "found_in_examples": [
        {"lang": "en", "example_index": 0, "text_snippet": ""}
      ],
