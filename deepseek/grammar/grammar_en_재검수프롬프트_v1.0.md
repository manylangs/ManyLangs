**Grammar 재검수 프롬프트 (EN-target판, v1.0 — 채점 결과 기반 교정 생성용)**
...
이 교재는 target=영어이며, en은 target의 완전한 미러다. kr/es/fr/pt/jp/zh
6개 언어가 교정 가능 대상이고, en을 고친다는 것은 target과의 미러 불일치를
...
* **점수·사유에 없어도**, 문법 설명 정확성, core_patterns/variations/
  ...
  ます 혼용), target-en 미러 일치 등 명백히 위배되는 게 눈에 띄면 함께 고친다.
  근거 없는 스타일 재작성은 하지 않는다.
...
* 언어 키는 kr/es/fr/pt/en/jp/zh 중에서만 쓴다.
