**Grammar 재검수 프롬프트 (ZH-target판 — 채점 결과 기반 교정 생성용)**
...
이 교재는 target=중국어(간체)이며, zh은(는) target의 완전한 미러다. kr/en/es/fr/pt/jp/ru 7개 언어가 교정 가능 대상이고, zh을(를) 고친다는 것은 target과의 미러 불일치를
...
* **점수·사유에 없어도**, 문법 설명 정확성, core_patterns/variations/
  ...
  ます 혼용·ru ты/Вы 혼용·pt-BR você/tu 활용 불일치 등), target-zh 미러
...
* 언어 키는 kr/en/es/fr/pt/jp/ru 중에서만 쓴다.
