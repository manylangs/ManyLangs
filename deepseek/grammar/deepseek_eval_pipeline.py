#!/usr/bin/env python3
"""
grammar_eval_pipeline.py

grammar generator/data (review.py와 동일한 원본 경로)를 순회하며
DeepSeek로 채점하고, 결과를 파일로 저장하지 않고 터미널에
batch_id 순서대로 출력한다 (voca/real/conversation용 eval_pipeline과 동일한 구조).

grammar 시리즈는 conversation처럼 target/미러 개념이 있다 (kr판/en판,
--target-lang 필수). domain_scores는 dict 형태, weight 합계는 100.
review.py의 교체 키는
  REPLACEMENTS: (block_index 1-17, lang) -> "문장"
  TITLE_REPLACEMENTS: {lang: "제목"}
blocking_issues 객체는 {domain, block_index, lang, issue} (set_id/line_number 없음,
block_index 하나만 있는 점이 conversation과 다름).
...
DEFAULT_ROOT = Path(
    "/Users/junghasuk/Desktop/ManyLangs/web/firebase/grammar generator/data"
)
...
# ---------------------------------------------------------------------------
# 1. 배치 스캔 — review.py의 resolve_json_file()과 동일한 경로
# ---------------------------------------------------------------------------
...
def scan_batches(root: Path, batch_filter):
    ...
    for batch_dir in sorted(root.iterdir()):
        ...
        if batch_filter and batch_id not in batch_filter:
            continue
        fp = batch_dir / f"grammar_{batch_id}.runtime.json"
        if fp.is_file():
...
# ---------------------------------------------------------------------------
# 3. 검증 및 게이트 (domain_scores dict, blocking_issues 객체: block_index/lang)
# ---------------------------------------------------------------------------
...
def build_review_block_text(target_lang: str, batch_id: str, batch_json: dict, result: dict) -> str:
    lines = []
    lines.append(f"# 재검수 요청 — series=grammar target={target_lang} batch_id={batch_id}")
    lines.append("")
    ...
    if blocking:
        ...
        for b in blocking:
            lines.append(
                f"- [{b.get('domain')}] block_index={b.get('block_index')} "
                f"lang={b.get('lang')}: {b.get('issue')}"
            )
...
def run(root: Path, prompt_dir: Path, prompt_file: str, target_lang: str, batch_filter,
    ...
    print(f"[스캔 결과] {len(entries)}개 배치 발견 (series=grammar target_lang={target_lang})")
