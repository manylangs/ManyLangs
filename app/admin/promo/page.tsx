...
import { PROMO_LANGUAGES, PROMO_PLATFORMS } from "./languages";
...
type CampaignType = "language" | "region" | "platform";
...
export default function AdminPromoPage() {
  const [campaignType, setCampaignType] =
    useState<CampaignType>("language");

  const [region, setRegion] = useState("BR");
  const [language, setLanguage] = useState("KOREAN");
  const [platform, setPlatform] = useState("SNS-TT");
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      if (campaignType === "language") {
        payload = {
          language,
          durationDays,
        };
      } else if (campaignType === "region") {
        payload = {
          region,
          durationDays,
        };
      ...
      >
        ...
        >
          ...
          >
            ...
            <option value="platform">SNS Platform</option>
          </select>
        ...
        >
          Format: Language = PROMO-MMDD-LANGUAGE · Region =
          PROMO-MMDD-REGION · SNS = PROMO-MMDD-SNS-PLATFORM
        </p>
      ...
      <div>
        ...
        {stats && (
          <>
            ...
            >
              {Object.entries(
                stats?.languageRegionStats ?? {}
              )
                .sort((a, b) => b[1] - a[1])
                .map(([r, count]) => {
                  const label =
                    PROMO_PLATFORMS.find(
                      (x) => x.code === r
                    )?.name ??
                    PROMO_LANGUAGES.find(
                      (x) => x.code === r
                    )?.name ??
                    REGIONS.find(
                      (x) => x.code === r
              ...
              >
                ...
                <tbody>
                  {stats?.campaigns?.map((c, i) => {
                    ...
                    const key =
                      c.platform ?? c.language ?? c.region;
